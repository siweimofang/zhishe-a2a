...
INDUSTRY_KW = frozenset([
    ...
    u"说明",
    # 室内设计基础批次回归修复(2026-08-14): "有哪些/怎么选"为口语高频问法词,
    # jieba切出后是唯一区分词,不加则"吊顶有哪些造型"类查询词集只剩{吊顶,造型}同分按库序垫底
    # k1687吊顶造型差1分输k461/k463/k692; k1699瓷砖/k1736浴缸与对手同分6按库序败
    u"有哪些", u"怎么选",
    ...
    u"朝北", u"对着",
    # 两本P0投喂修复(2026-08-14): "桦木"被jieba切成"桦/木"剔除,变体"桦木板做饰面板好不好"
    # 词集只剩{木板,饰面板}与k061/k079同分输库序;补整词后k1770 question+3/answer+1独高
    u"桦木",
    # 两本P0投喂修复(2026-08-14): "PVC"字母串不在中文分词范围,变体"PVC地板能用在家里吗"
    # 词集只剩{地板,家里,能用}全输给k331(地暖地板);补整词后k1785 question+3/answer+1/tags+2
    u"PVC",
    # 两本P0投喂修复(2026-08-14): "回风"被jieba切单字"回/风"剔除,变体"中央空调回风方式有哪些"
    # 词集只剩{中央空调,方式,有哪些}输给照明/软包方式条目;补整词后k1765 question+3/answer+1/tags+2
    u"回风",
    # IP法则k1813修复(2026-08-14): "OKR""KPI"字母串不在中文分词范围,变体"OKR和KPI怎么区分"
    # 词集只剩{区分}与k1105/k1237/k1333同分输库序;全库仅k1813出现,补整词后question+6独高
    u"OKR", u"KPI",
    # 材料清单批次回归补强(2026-08-14): jieba全单字切碎致query词集为空无法检索
    # 过道12条/烤漆5条/混搭4条/靠谱6条/找平2条,补整词后词集非空可命中
    u"过道", u"烤漆", u"混搭", u"靠谱", u"找平",
    # 材料清单批次回归补强(2026-08-14): jieba丢核心字致强实义词检索失效
    # 软木地板/实木地板丢"实/软"只剩"木地板"与65条地板条目同分输库序;室内门丢"门"只剩"室内"泛命中;
    # 新中式丢"新"与"中式"同分;门槛石丢"石";硅藻泥丢"泥";检修口丢"口"
    u"软木地板", u"实木地板", u"室内门", u"新中式", u"门槛石", u"硅藻泥", u"检修口",
    # 尺寸剖析2本投喂修复(2026-08-16): jieba未收录"和室/地窗"切成单字剔除致
    # "和室地窗做多高"词集为空返回空结果;补整词后词集非空可命中k2396,和室类问法一并受益
    u"和室", u"地窗",
    # 尺寸3本投喂回归修复(2026-08-16): jieba切"多宽/多深/多高"为单字"多/宽"剔除致
    # "卫生间门做多宽"词集只剩{卫生间}、"玄关鞋柜做多深"只剩{玄关,鞋柜}、"厨房吊柜做多高"只剩{厨房,吊柜}
    # 同分按库序垫底;补整词后各条目question命中+3独高;"低柜台"被切"低/柜台"丢核心词同理补整词
    u"多宽", u"多深", u"多高", u"低柜台",
    # 爆款视频号批回归修复(2026-08-17): jieba切"被骂"为"被/骂"单字剔除致
    # "视频被骂是不是就废了"词集只剩{视频}与全库视频条目同分按库序垫底,k3285排不进top3;
    # "火"切单字剔除致"爆款视频靠什么火"词集只剩{爆款,视频}与12分标签条目平分,k3295同8分垫底
    u"被骂", u"火",
    # GB50327隐蔽工程批回归修复(2026-08-18): jieba切"穿管"为"穿/管"单字剔除致
    # "电线为什么要穿管"词集只剩{电线}与含"电线"旧条目同分按库序垫底,k3428排不进top3;
    # 补整词后k3428的question命中+3独高;"穿线"同源词一并补上防"穿线根数"问法同类问题
    u"穿管", u"穿线",
    # GB50327防火饰面批回归修复(2026-08-18): jieba切"多干"为"多/干"单字剔除致
    # "刷乳胶漆前墙面要多干"词集只剩{乳胶漆,墙面}与刷漆旧条目同分按库序垫底,k3462排不进top3;
    # "喷漆枪离墙面多远"切"喷漆/枪离"错位致k3464仅{喷漆}命中输给含"喷漆"旧条目;补整词后命中提升
    u"多干", u"喷漆枪",
    # GB50210批1回归修复(2026-08-18): jieba切"冻裂"为"冻/裂"单字剔除致
    # "北方冬天埋在墙里的水管会冻裂吗"词集只剩{北方,冬天,水管}与k025(北方+装修)同分按库序被反超,
    # k3484排不进top3;补整词后k3484 question/answer命中提升,同问法"防冻"一并受益
    u"冻裂",
])
